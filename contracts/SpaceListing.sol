...
import { ISpaceExchange, Decimal } from "./interfaces/ISpaceExchange.sol";
import { ISpace } from "./interfaces/ISpace.sol";
import { ISpaceListing } from "./interfaces/ISpaceListing.sol"; 
...
interface ISpaceExtended is ISpace {
    function spaceExchangeContract() external returns(address);
}
 
contract SpaceListing is ISpaceListing, ReentrancyGuard {
    ...
    mapping(uint256 => ISpaceListing.Listing) public listings; 
    
    bytes4 constant interfaceId = 0x01ffc9a7;  
   ...
   function _approveListingDrop(uint256 listingId, bool approved, uint256 startsAt) internal { 
    ...
    function _handleMotifListingSettlement(uint256 listingId) internal returns (bool, uint256) {
        ...
        ISpaceExchange.Bid memory bid = ISpaceExchange.Bid({
        ...
        IERC20(currency).approve(ISpaceExtended(motif).spaceExchangeContract(), bid.amount);
        ISpace(motif).setBid(listings[listingId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try ISpace(motif).acceptBid(listings[listingId].tokenId, bid) {} catch { 
            ISpaceExtended(motif).removeBid(listings[listingId].tokenId);
            return (false, 0);
        }
