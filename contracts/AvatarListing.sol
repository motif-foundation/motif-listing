...
import { IAvatarExchange, Decimal } from "./interfaces/IAvatarExchange.sol";
import { IAvatar } from "./interfaces/IAvatar.sol";
import { IAvatarListing } from "./interfaces/IAvatarListing.sol"; 
...
interface IAvatarExtended is IAvatar {
    function avatarExchangeContract() external returns(address);
}
 
contract AvatarListing is IAvatarListing, ReentrancyGuard {
    ...
    mapping(uint256 => IAvatarListing.Listing) public listings; 
   ...
   function _approveListingDrop(uint256 listingId, bool approved, uint256 startsAt) internal { 
    ...
    function _handleMotifListingSettlement(uint256 listingId) internal returns (bool, uint256) {
        ...
        IAvatarExchange.Bid memory bid = IAvatarExchange.Bid({
        ...
        IERC20(currency).approve(IAvatarExtended(motif).avatarExchangeContract(), bid.amount);
        IAvatar(motif).setBid(listings[listingId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try IAvatar(motif).acceptBid(listings[listingId].tokenId, bid) {} catch { 
            IAvatarExtended(motif).removeBid(listings[listingId].tokenId);
            return (false, 0);
        }
