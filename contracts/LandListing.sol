...
import { ILandExchange, Decimal } from "./interfaces/ILandExchange.sol";
import { ILand } from "./interfaces/ILand.sol";
import { ILandListing } from "./interfaces/ILandListing.sol"; 
...
interface ILandExtended is ILand {
    function landExchangeContract() external returns(address);
}
 
contract LandListing is ILandListing, ReentrancyGuard {
    ...
    mapping(uint256 => ILandListing.Listing) public listings; 
...
function createMultipleListings(
   ...
   function _approveListingDrop(uint256 listingId, bool approved, uint256 startsAt) internal { 
    ...
    function _handleMotifListingSettlement(uint256 listingId) internal returns (bool, uint256) {
        ...
        ILandExchange.Bid memory bid = ILandExchange.Bid({
        ...
        IERC20(currency).approve(ILandExtended(motif).landExchangeContract(), bid.amount);
        ILand(motif).setBid(listings[listingId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try ILand(motif).acceptBid(listings[listingId].tokenId, bid) {} catch { 
            ILandExtended(motif).removeBid(listings[listingId].tokenId);
            return (false, 0);
        }
